...
def test_remove_student_view_positive(course_title):
    """
    Positive test: The teacher who owns the course can remove an enrolled student.
    """
    client = APIClient()
    ...
    data = {"is_active": False, "student": student.id, "course": course.id}
    response = client.put(url, data, format="json")
    assert (
        response.status_code == 200
    ), f"Expected 200, got {response.status_code}, Response: {response.json()}"
    enrollment.refresh_from_db()
    assert not enrollment.is_active, "Enrollment should be inactive after removal."


@pytest.mark.django_db
def test_remove_student_view_negative_not_owner():
    """
    Negative test: Another teacher (not the course owner) tries to remove a student.
    """
    client = APIClient()
...
@pytest.mark.django_db
def test_remove_student_view_edge_case_non_existent_enrollment():
    """
    Edge case: Attempt to remove a student from a non-existent enrollment should return 404.
    """
    client = APIClient()
    teacher = CustomUser.objects.create_user(
